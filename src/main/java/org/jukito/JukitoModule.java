...
public abstract class JukitoModule extends TestModule {
	 ...
	 * {@link TestMockSingleton} are automatically mocked. Use
	 * {@link #forceMock} to indicate that all concrete classes derived from the
	 * a specific type should be mocked in {@link org.jukito.TestMockSingleton}
	 * scope.
	 * 
	 * @param klass
	 *            The {@link Class} or interface for which all subclasses will
	 *            be mocked.
	 */
	...
	public final void configure() {
				...
				if (!keysObserved.contains(key)) {
					if (subClass.isAnnotationPresent(TestSingleton.class)) {
						...
						keysObserved.add(key);
							...
							.isAnnotationPresent(TestEagerSingleton.class)) {
						bind(subClass).in(TestScope.EAGER_SINGLETON);
						keysObserved.add(key);
					} else if (subClass
							.isAnnotationPresent(TestMockSingleton.class)) {
		...
		while (currentClass != null) {
			for (Method method : currentClass.getDeclaredMethods()) {
				if (method.isAnnotationPresent(Test.class)
					...
					List<Key<?>> keys = GuiceUtils
							.getMethodKeys(method, errors);

					for (Key<?> key : keys) {
						// Skip keys annotated with @All
						if (!All.class.equals(key.getAnnotationType())) {
							Key<?> keyNeeded = GuiceUtils.ensureProvidedKey(
									key, errors);
		...
		for (int i = 0; i < bindingsObserved.size(); ++i) {
			...
			if (!bindingInfo.isBoundToInstanceOrMock) {
				if (bindingInfo.boundType != null) {
					addDependencies(Key.get(bindingInfo.boundType),
							keysObserved, keysNeeded);
				} else {
					addDependencies(Key.get(bindingInfo.abstractType),
							keysObserved, keysNeeded);
				}
			}
		}
		...
		for (Key<?> key : keysNeeded) {
			...
			if (!keysObserved.contains(key) && !isCoreGuiceType(rawType)
				...
				super.bind(key).toProvider(new MockProvider(rawType))
	...
	protected <T> AnnotatedBindingBuilder<T> bind(TypeLiteral<T> typeLiteral) {
		return new SpyAnnotatedBindingBuilder<T>(
				newBindingObserved(typeLiteral), super.bind(typeLiteral));
	}
	...
	private <T> void addDependencies(Key<T> key, Set<Key<?>> keysObserved,
		...
		addInjectionPointDependencies(InjectionPoint.forConstructorOf(type),
				keysObserved, keysNeeded);
		Set<InjectionPoint> methodsAndFieldsInjectionPoints = InjectionPoint
				.forInstanceMethodsAndFields(type);
		for (InjectionPoint injectionPoint : methodsAndFieldsInjectionPoints) {
			addInjectionPointDependencies(injectionPoint, keysObserved,
					keysNeeded);
		}
	}
	...
	private void addKeyDependency(Key<?> key, Set<Key<?>> keysObserved,
		...
		if (Provider.class.isAssignableFrom(key.getTypeLiteral().getRawType())) {
			Type providedType = ((ParameterizedType) key.getTypeLiteral()
	...
	private static class SpyLinkedBindingBuilder<T> implements
		...
		public ScopedBindingBuilder toProvider(Provider<? extends T> provider) {
			...
			if (SpyProvider.class.isAssignableFrom(providerClass)) {
				...
				bindingInfo.boundType = AssistedInjectHelper
						.getProvidedType(provider);
			} else {
				bindingInfo.isBoundToInstanceOrMock = true;
			}
